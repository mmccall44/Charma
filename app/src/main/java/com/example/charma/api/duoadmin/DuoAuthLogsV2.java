...
import com.example.charma.api.duoclient.*;
...
public class DuoAuthLogsV2 {
    ...
    /**
     * The main function for the DuoAdmin example that will get auth logs.
     *
    ...
    private static void getLogsWithPaging(CommandLine cmd) {
        ...
        try {
            // Prepare request.
            Admin request = new
                    Admin.AdminBuilder("GET", cmd.getOptionValue("host"), "/admin/v2/logs/authentication")
                    .build();
            String limit = "2";
            ...
            while (hasMoreLogs) {
                ...
                if (!metadata.isNull("next_offset")) {
                    ...
                    request = new
                            Admin.AdminBuilder("GET", cmd.getOptionValue("host"), "/admin/v2/logs/authentication")
                            .build();
                    nextoffset = offset.get(0).toString() + ',' + offset.get(1).toString();
                    request.addParam("next_offset", nextoffset);
                    request.addParam("limit", limit);
                    request.addParam("mintime", Long.toString(mintime));
                    request.addParam("maxtime", Long.toString(maxtime));
                    request.signRequest(cmd.getOptionValue("ikey"), cmd.getOptionValue("skey"));

                    System.out.println("More to fetch: next_offset = " + nextoffset);
                } else {
                    hasMoreLogs = false;
